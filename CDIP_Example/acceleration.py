...
def process(fn:str, args:argparse.ArgumentParser) -> None:
        ...
        xyz["ax"] = calcAcceleration(xyz.x.to_numpy(), fs)
        xyz["ay"] = calcAcceleration(xyz.y.to_numpy(), fs)
        xyz["az"] = calcAcceleration(xyz.z.to_numpy(), fs)
        print(xyz)
        print(xyz.ax)
        ...
        for i in range(args.skip, min(args.skip + args.n, wave.t.size)):
            ...
            x = xyz.ax[q].to_numpy() # x is northward
            y = xyz.ay[q].to_numpy() # y is eastward
            z = xyz.az[q].to_numpy() # z is upwards
