...
def process(fn:str, args:argparse.ArgumentParser) -> None:
        ...
        xyz["vx"] = calcVelocity(xyz.x.to_numpy(), fs)
        xyz["vy"] = calcVelocity(xyz.y.to_numpy(), fs)
        xyz["vz"] = calcVelocity(xyz.z.to_numpy(), fs)
        ...
        for i in range(args.skip, min(args.skip + args.n, wave.t.size)):
            ...
            x = xyz.vx[q].to_numpy() # x is northward
            y = xyz.vy[q].to_numpy() # y is eastward
            z = xyz.vz[q].to_numpy() # z is upwards
