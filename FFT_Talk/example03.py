#! /usr/bin/env python3
#
# Make a set of PNG plots of a Fourier transform of a velocity wave.
#
...
zCurve  = args.amplitude * omega * np.sin(-omega * tCurve  + np.radians(args.phase))
zSample = args.amplitude * omega * np.sin(-omega * tSample + np.radians(args.phase))
...
fft[1:] /= fftOmega[1:] # Drop out omega term to get original A term, ignore DC term
fftAmp = np.abs(fft)
